...
import { 
  Container, Typography, Box, Button, Card, Chip, CircularProgress, IconButton,
...
export default function FilaChamadosPage() {
  ...
  const fetchChamados = async () => {
    ...
    try {
      ...
      if (user?.categoria?.id) {
        lista = lista.filter(c => c.categoria?.id === user.categoria.id);
      }
      ...
      console.error(error);
      show('Erro ao carregar fila.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const chamadosFiltrados = chamados.filter(c => {
    ...
    const matchTexto = 
      ...
      getSolicitanteName(c).toLowerCase().includes(termo);
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
        <Box>
          ...
          {user?.categoria && (
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 0.5 }}>
              Setor Vinculado: <strong>{user.categoria.nome}</strong>
            </Typography>
          )}
        </Box>
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <Tooltip title="Atualizar">
            <IconButton onClick={fetchChamados} color="primary" sx={{ bgcolor: 'white', border: '1px solid #e0e0e0' }}>
              <RefreshIcon />
            </IconButton>
          </Tooltip>
      ...
      {loading ? (
        ...
        <Card sx={{ p: 6, textAlign: 'center', bgcolor: '#f8f9fa' }}>
            <Typography color="text.secondary">Nenhum chamado encontrado.</Typography>
        </Card>
      ) : (
            ...
            <TableBody>
              {chamadosFiltrados.map((chamado) => (
                <TableRow 
                  ...
                  onClick={() => router.push(`/atendente/chamados/${chamado.id}`)}
                >
                  ...
                  <TableCell align="right">
                    <Tooltip title="Atender">
                      <IconButton size="small" sx={{ color: '#3182ce' }} onClick={(e) => { e.stopPropagation(); router.push(`/atendente/chamados/${chamado.id}`); }}>
